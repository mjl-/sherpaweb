...
const popupSmallStyle = {
    left: 'calc(50% - 200px)',
    right: 'calc(50% - 200px)',
};
const popupMediumStyle = {
    left: 'calc(50% - 300px)',
    right: 'calc(50% - 300px)',
};
const popupLargeStyle = {
    left: 'calc(50% - 450px)',
    right: 'calc(50% - 450px)',
};
export class Popup {
    constructor(app, view, size = PopupSize.Medium) {
        ...
        const looksPopup = size === PopupSize.Small ? app.ensureLooks('popup-small', popupStyle, popupSmallStyle) :
        ...
        this.closedResolve = (v) => { };
        this.closed = new Promise(resolve => {
            this.closedResolve = resolve;
        });
        const closeButton = dom.div(looksPopupCloseBtn, { tabindex: '0' }, dom.listen('keydown', e => {
            if (e.key === 'Enter') {
                this.cancel();
            }
        }), dom.listen('click', () => this.cancel()), 'x');
        this.background = dom.div(looksPopupBg, dom.listen('click', () => this.cancel()));
        this.popup = dom.div(looksPopup, closeButton, view, dom.listen('keyup', ev => {
            if (ev.key === 'Escape') {
                ev.stopPropagation();
                this.cancel();
            }
